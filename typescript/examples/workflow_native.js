...
export async function run() {
  console.log("=== NativeBPM TS SDK: Workflow with Native Service Tasks ===");

  // 1. Build workflow as code (without WASM tasks)
  const workflow = new Workflow('native-demo', 'Workflow with Native Service Tasks');
  ...
  try {
    ...
    const definition = await client.definitions().deploy()
      .withID("native-demo")
      .withName("Workflow with Native Service Tasks")
      .withBPMN(bpmnXML)
      .send();
